...
public class TextTestActivity extends AppCompatActivity {
    //layout 페이지에서 사용할 view에 대한 리소스를 가져와야 하므로 변수 선언
    EditText txtinfo;
    TextView lblinfo;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnsetter.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                txtinfo.setText("가져온 물자열:작업완료");
            }
        });
        //EditText에서 데이터를 입력할때 이벤트 발생
        whatcherListener listener=new whatcherListener();
        txtinfo.addTextChangedListener(listener);
    }
    //TextChanged이벤트를 처리하는 리스너 작성
    class  whatcherListener implements TextWatcher{
